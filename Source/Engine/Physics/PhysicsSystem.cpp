...
namespace hop
{
	...
	{
		Vexctor2 worldPosition = ScreenToWorld(position);
	...
	void PhysicsSystem::SetCollisionBox(b2Body* body, const CollisionData& data, class Actor* actor)
	{
		...
		Vexctor2 worldSize = ScreenToWorld(data.size * 0.5f);
	...
	{
		Vexctor2 worldSize = PhysicsSystem::ScreenToWorld(data.size * 0.5f);
