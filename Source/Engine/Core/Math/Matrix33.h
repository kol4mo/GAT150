...
namespace hop
{
	...
	inline vec2 hop::Matrix33::operator*(const vec2& v)
	{
		// | a b c |   |x|
		// | d e f | * |y|
		// | g h i |   |1|
	...
	inline Matrix33 hop::Matrix33::operator*(const Matrix33& mx)
	{
		Matrix33 result;
		...
		result[2][1] = rows[2][0] * mx[0][2] + rows[2][1] * mx[1][2] + rows[2][2] * mx[2][2];

		return result;
	}

	inline Matrix33 hop::Matrix33::CreateTranslation(const vec2& translate)
	{
		Matrix33 mx = CreateIdentity();
		mx[0][2] = translate.x;
		mx[1][2] = translate.y;
	...
	inline vec2 hop::Matrix33::GetTranslation() const
	{

		return {rows[0][2], rows[1][2]};
	}

	inline float hop::Matrix33::GetRotation() const
	{
		return std::atan2(rows[1][0], rows[0][0]);
	}

	inline vec2 hop::Matrix33::getScale() const
	{
